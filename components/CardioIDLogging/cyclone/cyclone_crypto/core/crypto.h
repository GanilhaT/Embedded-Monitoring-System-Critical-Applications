...
typedef error_list (*HashAlgoCompute)(const void *data, size_t length,
   uint8_t *digest);
...
typedef error_list (*CipherAlgoInit)(void *context, const uint8_t *key,
   size_t keyLen);
...
typedef error_list (*KemAlgoGenerateKeyPair)(const PrngAlgo *prngAlgo,
   void *prngContext, uint8_t *pk, uint8_t *sk);

typedef error_list (*KemAlgoEncapsulate)(const PrngAlgo *prngAlgo,
   void *prngContext, uint8_t *ct, uint8_t *ss, const uint8_t *pk);

typedef error_list (*KemAlgoDecapsulate)(uint8_t *ss, const uint8_t *ct,
   const uint8_t *sk);

//Common API for pseudo-random number generators (PRNG)
typedef error_list (*PrngAlgoInit)(void *context);

typedef error_list (*PrngAlgoSeed)(void *context, const uint8_t *input,
   size_t length);

typedef error_list (*PrngAlgoAddEntropy)(void *context, uint_t source,
   const uint8_t *input, size_t length, size_t entropy);

typedef error_list (*PrngAlgoRead)(void *context, uint8_t *output, size_t length);
