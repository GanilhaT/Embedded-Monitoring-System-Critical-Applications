...
error_list ecdsaWriteSignature(const EcdsaSignature *signature, uint8_t *data,
   size_t *length)
{
   error_list error;
   size_t k;
...
error_list ecdsaReadSignature(const uint8_t *data, size_t length,
...
__weak_func error_list ecdsaGenerateSignature(const PrngAlgo *prngAlgo, void *prngContext,
...
__weak_func error_list ecdsaVerifySignature(const EcDomainParameters *params,
