...
error_list asn1ReadTag(const uint8_t *data, size_t length, Asn1Tag *tag);
error_list asn1ReadSequence(const uint8_t *data, size_t length, Asn1Tag *tag);
error_list asn1ReadOctetString(const uint8_t *data, size_t length, Asn1Tag *tag);
error_list asn1ReadOid(const uint8_t *data, size_t length, Asn1Tag *tag);

error_list asn1ReadBoolean(const uint8_t *data, size_t length, Asn1Tag *tag,
   bool_t *value);

error_list asn1ReadInt32(const uint8_t *data, size_t length, Asn1Tag *tag,
   int32_t *value);

error_list asn1WriteTag(Asn1Tag *tag, bool_t reverse, uint8_t *data,
   size_t *written);

error_list asn1WriteInt32(int32_t value, bool_t reverse, uint8_t *data,
   size_t *written);

#if (MPI_SUPPORT == ENABLED)

error_list asn1ReadMpi(const uint8_t *data, size_t length, Asn1Tag *tag,
   Mpi *value);

error_list asn1WriteMpi(const Mpi *value, bool_t reverse, uint8_t *data,
   size_t *written);

#endif

error_list asn1CheckTag(const Asn1Tag *tag, bool_t constructed, uint_t objClass,
   uint_t objType);

error_list asn1CheckOid(const Asn1Tag *tag, const uint8_t *oid, size_t length);

error_list asn1DumpObject(const uint8_t *data, size_t length, uint_t level);
