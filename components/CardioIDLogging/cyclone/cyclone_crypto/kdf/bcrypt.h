...
error_list bcryptHashPassword(const PrngAlgo *prngAlgo, void *prngContext,
   uint_t cost, const char_t *password, char_t *hash, size_t *hashLen);

error_list bcryptVerifyPassword(const char_t *password, const char_t *hash);

error_list bcrypt(uint_t cost, const uint8_t *salt, const char_t *password,
   char_t *hash, size_t *hashLen);

error_list eksBlowfishSetup(BlowfishContext *context, uint_t cost,
