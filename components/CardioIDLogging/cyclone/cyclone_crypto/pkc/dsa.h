...
error_list dsaWriteSignature(const DsaSignature *signature, uint8_t *data, size_t *length);
error_list dsaReadSignature(const uint8_t *data, size_t length, DsaSignature *signature);

error_list dsaGenerateSignature(const PrngAlgo *prngAlgo, void *prngContext,
   const DsaPrivateKey *key, const uint8_t *digest, size_t digestLen,
   DsaSignature *signature);

error_list dsaVerifySignature(const DsaPublicKey *key,
