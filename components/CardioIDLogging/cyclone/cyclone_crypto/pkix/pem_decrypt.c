...
error_list pemDecryptPrivateKey(const char_t *input, size_t inputLen,
...
error_list pemDecryptMessage(const PemHeader *header, const char_t *password,
...
error_list pemFormatIv(const PemHeader *header, uint8_t *iv, size_t ivLen)
{
   error_list error;
   size_t i;
   char_t *end;
...
error_list pemKdf(const char_t *p, size_t pLen, const uint8_t *s, size_t sLen,
