...
error_list sshProcessChannelEvents(SshChannel *channel);
...
error_list sshProcessChannelData(SshChannel *channel, const uint8_t *data,
   size_t length);

error_list sshProcessChannelExtendedData(SshChannel *channel, uint32_t type,
   const uint8_t *data, size_t length);

error_list sshUpdateChannelWindow(SshChannel *channel, uint32_t windowSizeInc);
