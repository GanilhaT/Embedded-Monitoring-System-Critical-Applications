...
error_list sshSendKexDhInit(SshConnection *connection)
{
...
error_list sshSendKexDhReply(SshConnection *connection)
{
...
error_list sshFormatKexDhInit(SshConnection *connection, uint8_t *p,
   size_t *length)
{
#if (SSH_CLIENT_SUPPORT == ENABLED)
   error_list error;
...
error_list sshFormatKexDhReply(SshConnection *connection, uint8_t *p,
   size_t *length)
{
#if (SSH_SERVER_SUPPORT == ENABLED)
   error_list error;
...
error_list sshParseKexDhInit(SshConnection *connection, const uint8_t *message,
   size_t length)
{
#if (SSH_SERVER_SUPPORT == ENABLED)
   error_list error;
...
error_list sshParseKexDhReply(SshConnection *connection, const uint8_t *message,
   size_t length)
{
#if (SSH_CLIENT_SUPPORT == ENABLED)
   error_list error;
...
error_list sshParseKexDhMessage(SshConnection *connection, uint8_t type,
...
error_list sshComputeDhSharedSecret(SshConnection *connection)
{
   error_list error;
...
error_list sshDigestClientDhPublicKey(SshConnection *connection)
{
   error_list error;
   size_t n;
   uint8_t *buffer;
