...
error_list ethInit(NetInterface *interface);

void ethProcessFrame(NetInterface *interface, uint8_t *frame, size_t length,
   NetRxAncillary *ancillary);

error_list ethSendFrame(NetInterface *interface, const MacAddr *destAddr,
   uint16_t type, NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary);

error_list ethAcceptMacAddr(NetInterface *interface, const MacAddr *macAddr);
error_list ethDropMacAddr(NetInterface *interface, const MacAddr *macAddr);

error_list ethAttachLlcRxCalback(NetInterface *interface, LlcRxCallback callback,
   void *param);

error_list ethDetachLlcRxCalback(NetInterface *interface);

NetBuffer *ethAllocBuffer(size_t length, size_t *offset);

error_list macStringToAddr(const char_t *str, MacAddr *macAddr);
char_t *macAddrToString(const MacAddr *macAddr, char_t *str);
void macAddrToEui64(const MacAddr *macAddr, Eui64 *interfaceId);

void ethDumpHeader(const EthHeader *ethHeader);

error_list eui64StringToAddr(const char_t *str, Eui64 *eui64);
