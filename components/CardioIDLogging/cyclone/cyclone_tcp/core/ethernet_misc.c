...
error_list ethPadFrame(NetBuffer *buffer, size_t *length)
{
   error_list error;
   size_t n;
...
error_list ethEncodeVlanTag(NetBuffer *buffer, size_t *offset, uint16_t vlanId,
...
error_list ethDecodeVlanTag(const uint8_t *frame, size_t length, uint16_t *vlanId,
   uint16_t *type)
{
   VlanTag *vlanTag;
...
error_list ethCheckDestAddr(NetInterface *interface, const MacAddr *macAddr)
{
   error_list error;
   uint_t i;
...
void ethUpdateErrorStats(NetInterface *interface, error_list error)
{
   //Check error code
   switch(error)
   {
...
error_list ethCheckCrc(NetInterface *interface, const uint8_t *frame,
   size_t length)
{
   uint32_t crc;
