...
error_list socketSetTimeout(Socket *socket, systime_t timeout)
{
...
error_list socketSetTtl(Socket *socket, uint8_t ttl)
{
   //Make sure the socket handle is valid
   if(socket == NULL)
      return ERROR_INVALID_PARAMETER;
...
error_list socketSetMulticastTtl(Socket *socket, uint8_t ttl)
{
   //Make sure the socket handle is valid
   if(socket == NULL)
      return ERROR_INVALID_PARAMETER;
...
error_list socketSetDscp(Socket *socket, uint8_t dscp)
{
...
error_list socketSetVlanPcp(Socket *socket, uint8_t pcp)
{
...
error_list socketSetVlanDei(Socket *socket, bool_t dei)
{
#if (ETH_VLAN_SUPPORT == ENABLED)
   //Make sure the socket handle is valid
   if(socket == NULL)
...
error_list socketSetVmanPcp(Socket *socket, uint8_t pcp)
{
...
error_list socketSetVmanDei(Socket *socket, bool_t dei)
{
#if (ETH_VMAN_SUPPORT == ENABLED)
   //Make sure the socket handle is valid
   if(socket == NULL)
...
error_list socketEnableKeepAlive(Socket *socket, bool_t enabled)
{
...
error_list socketSetKeepAliveParams(Socket *socket, systime_t idle,
...
error_list socketSetTxBufferSize(Socket *socket, size_t size)
{
...
error_list socketSetRxBufferSize(Socket *socket, size_t size)
{
#if (TCP_SUPPORT == ENABLED)
   //Make sure the socket handle is valid
   if(socket == NULL)
...
error_list socketSetInterface(Socket *socket, NetInterface *interface)
{
   //Make sure the socket handle is valid
   if(socket == NULL)
      return ERROR_INVALID_PARAMETER;
...
error_list socketBind(Socket *socket, const IpAddr *localIpAddr,
   uint16_t localPort)
{
...
error_list socketConnect(Socket *socket, const IpAddr *remoteIpAddr,
   uint16_t remotePort)
{
   error_list error;
...
error_list socketListen(Socket *socket, uint_t backlog)
{
#if (TCP_SUPPORT == ENABLED)
   error_list error;
...
error_list socketSend(Socket *socket, const void *data, size_t length,
...
error_list socketSendTo(Socket *socket, const IpAddr *destIpAddr, uint16_t destPort,
...
error_list socketSendMsg(Socket *socket, const SocketMsg *message, uint_t flags)
{
   error_list error;
...
error_list socketReceive(Socket *socket, void *data,
...
error_list socketReceiveFrom(Socket *socket, IpAddr *srcIpAddr, uint16_t *srcPort,
...
error_list socketReceiveEx(Socket *socket, IpAddr *srcIpAddr, uint16_t *srcPort,
...
error_list socketReceiveMsg(Socket *socket, SocketMsg *message, uint_t flags)
{
   error_list error;
...
error_list socketGetLocalAddr(Socket *socket, IpAddr *localIpAddr,
   uint16_t *localPort)
{
   //Make sure the socket handle is valid
   if(socket == NULL)
...
error_list socketGetRemoteAddr(Socket *socket, IpAddr *remoteIpAddr,
   uint16_t *remotePort)
{
   //Make sure the socket handle is valid
   if(socket == NULL)
...
error_list socketShutdown(Socket *socket, uint_t how)
{
#if (TCP_SUPPORT == ENABLED)
   error_list error;
...
error_list socketPoll(SocketEventDesc *eventDesc, uint_t size, OsEvent *extEvent,
   systime_t timeout)
{
   error_list error;
   uint_t i;
...
error_list getHostByName(NetInterface *interface,
