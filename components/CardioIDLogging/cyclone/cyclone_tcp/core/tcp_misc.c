...
error_list tcpSendSegment(Socket *socket, uint8_t flags, uint32_t seqNum,
...
error_list tcpSendResetSegment(Socket *socket, uint32_t seqNum)
{
   error_list error;
...
error_list tcpRejectSegment(NetInterface *interface, IpPseudoHeader *pseudoHeader,
...
error_list tcpAddOption(TcpHeader *segment, uint8_t kind, const void *value,
   uint8_t length)
{
   error_list error;
   size_t i;
...
error_list tcpCheckSeqNum(Socket *socket, TcpHeader *segment, size_t length)
{
   bool_t acceptable;
...
error_list tcpCheckSyn(Socket *socket, TcpHeader *segment, size_t length)
{
...
error_list tcpCheckAck(Socket *socket, TcpHeader *segment, size_t length)
{
   uint_t n;
   uint_t ownd;
   uint_t thresh;
...
error_list tcpRetransmitSegment(Socket *socket)
{
   error_list error;
   size_t offset;
   size_t length;
...
error_list tcpNagleAlgo(Socket *socket, uint_t flags)
{
   error_list error;
   uint32_t n;
   uint32_t u;
...
error_list tcpReadTxBuffer(Socket *socket, uint32_t seqNum,
