...
error_list ipv6InitRouting(void)
{
...
error_list ipv6EnableRouting(NetInterface *interface, bool_t enable)
{
   //Check parameters
   if(interface == NULL)
...
error_list ipv6AddRoute(const Ipv6Addr *prefix, uint_t prefixLen,
...
error_list ipv6DeleteRoute(const Ipv6Addr *prefix, uint_t prefixLen)
{
   error_list error;
   uint_t i;
   Ipv6RoutingTableEntry *entry;
...
error_list ipv6DeleteAllRoutes(void)
{
   //Get exclusive access
   osAcquireMutex(&netMutex);
   //Clear the routing table
...
error_list ipv6ForwardPacket(NetInterface *srcInterface, NetBuffer *ipPacket,
