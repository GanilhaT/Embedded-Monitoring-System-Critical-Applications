...
typedef error_list (*PppRandCallback)(uint8_t *data, size_t length);
...
error_list pppInit(PppContext *context, const PppSettings *settings);

error_list pppSetTimeout(NetInterface *interface, systime_t timeout);

error_list pppSetAuthInfo(NetInterface *interface,
   const char_t *username, const char_t *password);

bool_t pppCheckPassword(NetInterface *interface, const char_t *password);

error_list pppSendAtCommand(NetInterface *interface, const char_t *data);
error_list pppReceiveAtCommand(NetInterface *interface, char_t *data, size_t size);

error_list pppConnect(NetInterface *interface);
error_list pppClose(NetInterface *interface);
...
error_list pppSendFrame(NetInterface *interface,
